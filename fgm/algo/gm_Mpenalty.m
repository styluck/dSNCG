function asg = gm_Mpenalty(K, asgT, opts)
% Graph matching using SMOOTH penalty method with BB steps
%
% Input
%   K         -  global kernel matrix, nn x nn
%   asgT      -  ground-truth assignment (can be [])
% Output
%   asg       -  assignment
%     alg     -  'fgmU'
%     X       -  correspondence matrix, n1 x n2
%     acc     -  accuracy
%     obj     -  objective

% function parameter
Ksnorm = svds(K,1);
[n, r] = size(asgT.X);

sqhfun = @(x)sum(x(x<0).^2);      % sum(sum(max(0,-x).^2))
hfun = @(x)abs(sum(x(x<0)));      % sum(sum(max(0,-x)))
retr = @(x,v) retr_st(x,v,r,3);
lmxitr = opts.lmxitr;

ha = tic;

% params
acc = -100;
obj= -100;

% subopts.record = 0;
% subopts.mxitr = 100;
% subopts.xtol = 1e-5;
% subopts.gtol = 1e-5;
% subopts.ftol = 1e-8;

accsq = zeros(lmxitr, 1);
objsq = zeros(lmxitr, 1);

% algorithm: gm_Mpenalty solver
for i =1:lmxitr
    randstate = 100+(i-1)*9867;
    randn('state',double(randstate));
    rand('state',double(randstate));
    state = rng;
    
    X0 = randn(n);    
    X0 = orth(X0);
    %X0 = OptStiefelGBB(X0, @objfun_QAP,subopts,K,n,r,2);
    [fobj,X_tmp] = Mpenalty_BB(X0,retr,hfun,opts,K,n,r);
    
    X_tmp = round(X_tmp);
    accsq(i) = matchAsg(X_tmp, asgT);
    objsq(i) = X_tmp(:)'*K*X_tmp(:);
% compare with ground-truth: the accuracy
    if accsq(i) > acc
        X = X_tmp;
        acc = accsq(i);
        obj = objsq(i); 
    end
end

% store
asg.acc = acc;
asg.alg = 'Mpenalty-BB';
asg.X = X;
asg.obj = obj;
asg.tim = toc(ha);
asg.accsq = accsq;
asg.objsq = objsq;
end

% [EOF]