...
for dn = 1:nlen
    ...
    %profile on;
    tic; 
    [X, out]= OptStiefelGBB(X0, @funeigsym, opts, A); 
    tsolve = toc;
    
    % profile viewer;
    out.fval = -2*out.fval;
    err = (feig-out.fval)/(abs(feig)+1);
    fprintf('wen''s: obj val %7.6e, cpu %f, #func eval %d, itr %d, |XT*X-I| %3.2e\n', ...
    ...
    %profile on;
    tic; 
    [X, out]= OptGrassmannGBB(X0, @funeigsym, opts, A); 
    tsolve2 = toc;
    
    % profile viewer;
    out.fval = -2*out.fval;
    err = (feig-out.fval)/(abs(feig)+1);
    fprintf('our''s: obj val %7.6e, cpu %f, #func eval %d, itr %d, |XT*X-I| %3.2e\n', ...
             out.fval, tsolve2, out.nfe, out.itr, norm(X'*X - eye(k), 'fro'));
    fprintf('relative difference between two obj vals: %3.2e\n',...
         err);
    out.feasi = norm(X'*X - eye(k), 'fro');
